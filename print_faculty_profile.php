                                ...
                                <?php
                                    ...
                                    $fetch = $display->fetchAll(); 
                                        foreach ($fetch as $key => $row) {
                                            $dept_id = $row['dept_id'];
                                            $dept_name = $row['dept_name'];
                                    $bsfilter1 = $con->prepare("SELECT COUNT(bs_name) as bstotal1 FROM tbl_personnel WHERE dept_id = '$dept_id' AND bs_name <> ' ' AND per_designation in ('Permanent' , 'Part Time')");
                                    $bsfilter1->execute();
                                    $bsfetch = $bsfilter1->fetch(); 
                                        $bstotal1 = $bsfetch['bstotal1'];
                                    $msfilter1 = $con->prepare("SELECT COUNT(ms_name) as count FROM tbl_personnel WHERE dept_id = '$dept_id' AND ms_name <> ' ' AND per_designation in ('Permanent' , 'Part Time')");
                                    $msfilter1->execute();
                                    $msfetch = $msfilter1->fetch(); 
                                    $mstotal1 = $msfetch['count'];

                                    $drfilter1 = $con->prepare("SELECT COUNT(dr_name) as count FROM tbl_personnel WHERE dept_id = '$dept_id' AND dr_name <> ' '  AND per_designation in ('Permanent' , 'Part Time')");
                                    $drfilter1->execute();
